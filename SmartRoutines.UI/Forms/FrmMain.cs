...
namespace SmartRoutines.UI.Forms
{
    public partial class FrmMain : Form
    {
        ...
        {
            if (_currentPage is Controls.UC_Dashboard dash)
            {
                dash.AddNewRoutine();
            }
            else
            {
                // If they are on another page, navigate back to dashboard first
                btnNavDashboard_Click(this, EventArgs.Empty);
                if (_currentPage is Controls.UC_Dashboard newDash)
                {
                    newDash.AddNewRoutine();
                }
            }
        }
