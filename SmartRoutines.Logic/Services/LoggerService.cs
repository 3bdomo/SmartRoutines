...
public sealed class LoggerService : IActivityLogService
{
    ...
    //}
    public LoggerService(IServiceScopeFactory scopeFactory)
    {
    ...
    public async Task AddLogAsync(ActivityLogDto logDto)
    {
        ...
        // ✅ عمل Scope جديد لكل عملية
        using var scope = _scopeFactory.CreateScope();
        var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();

        var entity = new ActivityLog(
            Guid.Empty,
            string.IsNullOrWhiteSpace(logDto.RoutineName) ? "Unnamed Routine" : logDto.RoutineName,
            logDto.Status,
            logDto.Message ?? string.Empty);

        await unitOfWork.ActivityLogs.AddAsync(entity);
    ...
    public async Task ClearAllLogsAsync()
    {
        ...
        await unitOfWork.ActivityLogs.ClearAllAsync();
        await unitOfWork.SaveChangesAsync();

        OnLogsCleared?.Invoke(this, EventArgs.Empty);
    }

    public async Task<(IReadOnlyList<ActivityLogDto> Logs, int TotalCount)> GetPagedLogsAsync(
        int page, int pageSize, LogStatus? statusFilter = null, Guid? routineId = null)
    {
        page = page <= 0 ? 1 : page;
        pageSize = pageSize <= 0 ? 10 : pageSize;
        ...
        var totalCount = await unitOfWork.ActivityLogs.GetTotalCountAsync(statusFilter);
        var entities = (await unitOfWork.ActivityLogs.GetPagedAsync(page, pageSize, statusFilter, routineId)).ToList();

        var dtos = entities.Select(e => new ActivityLogDto
        {
            Id = e.Id,
            RoutineName = e.RoutineName,
            Status = e.Status,
    ...
    public async Task LogActionResultAsync(ActionEntry action, Core.Domain.Models.ActionContext context, LogStatus status, string message)
    {
        ...
        var routineName = string.IsNullOrWhiteSpace(context.RoutineName)
            ? "Unnamed Routine" : context.RoutineName;

        var log = new ActivityLog(Guid.Empty, routineName, status, $"[{action.Type}] {message}");
    ...
    private string GetRelativeTime(DateTime dateTime)
    {
        ...
        if (diff.TotalDays < 7) return $"{(int)diff.TotalDays} day ago";
        return dateTime.ToString("yyyy-MM-dd");
    }
}
